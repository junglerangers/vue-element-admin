...
module.exports = {
  ...
  // productionGZip:true,
  // 前端服务器
  devServer: { // 等价于webpack devServer,支持热重载,允许你在运行服务器时动态修改代码
    port: port, // 端口号
    open: true, // 打开你的默认浏览器
    hot: process.env.NODE_ENV === 'development', // hot reload.
    overlay: { // 显示错误信息,警告不展示,错误展示
      warnings: false,
      errors: true
    },
  ...
  chainWebpack(config) { // 链式webpack
    ...
    config
      .when(process.env.NODE_ENV !== 'development', // 只会在非开发环境下才会生效的配置
        config => {
          config
            ...
            .end()
          config
            .optimization.splitChunks({ // 优化,使用一个名为splicChunks的插件
              chunks: 'all', // include all of chunks. -- 在所有chunk中分离出来的单独一个公用chunk
              cacheGroups: { // 缓存组,将该部分的chunk缓存到用户的网站中,减少每次访问时需要加载的数量
                libs: { // 创建的一个chunk
                  name: 'chunk-libs',
                  test: /[\\/]node_modules[\\/]/,
                  priority: 10, // 优先级
